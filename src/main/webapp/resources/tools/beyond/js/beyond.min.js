...
function getThemeColorFromCss(c) {
    var a = $("<span></span>").hide().appendTo("body");
    a.addClass(c);
    var b = a.css("color");
    a.remove();
    return b
...
$("#btn-setting").on("click", function (a) {
    $(".navbar-account").toggleClass("setting-open")
});
$("#fullscreen-toggler").on("click", function (b) {
...
function InitiateSideMenu() {
    ...
    var a = $("#sidebar").hasClass("menu-compact");
    $("#sidebar-collapse").on("click", function () {
        ...
        a = $("#sidebar").hasClass("menu-compact");
        ...
        if (a) {
            ...
            if ($(".page-sidebar").hasClass("sidebar-fixed")) {
                var b = (readCookie("rtl-support") || location.pathname == "/index-rtl-fa.html" || location.pathname == "/index-rtl-ar.html") ? "right" : "left";
                $(".sidebar-menu").slimscroll({height: "auto", position: b, size: "3px", color: themeprimary})
            }
        }
    });
    $(".sidebar-menu").on("click", function (g) {
        var d = $(g.target).closest("a");
        if (!d || d.length == 0) {
            return
        }
        if (!d.hasClass("menu-dropdown")) {
            if (a && d.get(0).parentNode.parentNode == this) {
                var f = d.find(".menu-text").get(0);
                if (g.target != f && !$.contains(f, g.target)) {
                    return false
                }
            }
            return
        ...
        var b = d.next().get(0);
        if (!$(b).is(":visible")) {
            var h = $(b.parentNode).closest("ul");
            if (a && h.hasClass("sidebar-menu")) {
                return
            }
            h.find("> .open > .submenu").each(function () {
                if (this != b && !$(this.parentNode).hasClass("active")) {
                    $(this).slideUp(200).parent().removeClass("open")
                }
            })
        }
        if (a && $(b.parentNode.parentNode).hasClass("sidebar-menu")) {
            return false
        }
...
function InitiateWidgets() {
    $('.widget-buttons *[data-toggle="maximize"]').on("click", function (d) {
        d.preventDefault();
        var e = $(this).parents(".widget").eq(0);
        var b = $(this).find("i").eq(0);
        var a = "fa-compress";
        var c = "fa-expand";
        if (e.hasClass("maximized")) {
            if (b) {
                b.addClass(c).removeClass(a)
            }
            e.removeClass("maximized");
            e.find(".widget-body").css("height", "auto")
        } else {
            if (b) {
    ...
    $('.widget-buttons *[data-toggle="collapse"]').on("click", function (d) {
        d.preventDefault();
        var e = $(this).parents(".widget").eq(0);
        var b = e.find(".widget-body");
        var c = $(this).find("i");
        var h = "fa-plus";
        var a = "fa-minus";
        var f = 300;
        ...
        if (e.hasClass("collapsed")) {
            if (c) {
                c.addClass(a).removeClass(h)
            }
            e.removeClass("collapsed");
            b.slideUp(0, function () {
                b.slideDown(f)
            })
        } else {
            if (c) {
                c.addClass(h).removeClass(a)
            }
            b.slideUp(g, function () {
                e.addClass("collapsed")
            })
        }
    });
    $('.widget-buttons *[data-toggle="dispose"]').on("click", function (b) {
        b.preventDefault();
        var d = $(this);
        var c = d.parents(".widget").eq(0);
        var a = 300;
        c.hide(a, function () {
            c.remove()
        })
...
function maximize(b) {
    if (b) {
        var c = $(window).height();
        var a = b.find(".widget-header").height();
        b.find(".widget-body").height(c - a)
    }
}
function scrollTo(b, a) {
    var c = (b && b.size() > 0) ? b.offset().top : 0;
    jQuery("html,body").animate({scrollTop: c + (a ? a : 0)}, "slow")
}
function Notify(d, a, e, f, c, b) {
    toastr.options.positionClass = "toast-" + a;
    toastr.options.extendedTimeOut = 0;
    toastr.options.timeOut = e;
    toastr.options.closeButton = b;
    toastr.options.iconClass = c + " toast-" + f;
    toastr.custom(d)
}
    ...
    if (readCookie("sidebar-fixed") != null) {
        if (readCookie("sidebar-fixed") == "true") {
            ...
            if (!$(".page-sidebar").hasClass("menu-compact")) {
                var a = (readCookie("rtl-support") || location.pathname == "/index-rtl-fa.html" || location.pathname == "/index-rtl-ar.html") ? "right" : "left";
...
$(".page-chatbar .chatbar-contacts .contact").on("click", function (a) {
    $(".page-chatbar .chatbar-contacts").hide();
    $(".page-chatbar .chatbar-messages").show()
});
$(".page-chatbar .chatbar-messages .back").on("click", function (a) {
...
function switchClasses(b, d) {
    var c = document.getElementsByClassName(b);
    for (i = c.length - 1; i >= 0; i--) {
        if (!hasClass(c[i], "dropdown-menu")) {
            addClass(c[i], b + "-temp");
            removeClass(c[i], b)
        }
    }
    ...
    for (i = a.length - 1; i >= 0; i--) {
        if (!hasClass(a[i], "dropdown-menu")) {
            addClass(a[i], b);
            removeClass(a[i], d)
        }
    }
    tempClasses = document.getElementsByClassName(b + "-temp");
    for (i = tempClasses.length - 1; i >= 0; i--) {
        if (!hasClass(tempClasses[i], "dropdown-menu")) {
            addClass(tempClasses[i], d);
            removeClass(tempClasses[i], b + "-temp")
        }
    }
}
...
function removeClass(b, a) {
    var c = " " + b.className + " ";
    b.className = c.replace(" " + a, "").replace(/^\s+/g, "").replace(/\s+$/g, "")
}
